...
class BasicDiscriminator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        feat.add_module('initial-conv-{0}-{1}'.format(nc, ndf),
                        nn.Conv2d(nc, ndf, 4, 2, 1, bias=False))
        # feat.add_module('initial-conv-{0}-{1}'.format(nc, ndf),
        #                 DepthwiseSeparableConv(nc, ndf, kernel_size=4,
        #                                        kernels_per_layer=1,
        #                                        padding=1))
        ...
        for t in range(n_extra_layers):
            feat.add_module('extra-layers-{0}-{1}-conv'.format(t, cndf),
                            nn.Conv2d(cndf, cndf, 3, 1, 1, bias=False))
            # feat.add_module('extra-layers-{0}-{1}-conv'.format(nc, ndf),
            #                 DepthwiseSeparableConv(cndf, cndf, kernel_size=3,
            #                                        kernels_per_layer=1,
            #                                        padding=1))
        ...
        while csize > 4:
            ...
            feat.add_module('pyramid-{0}-{1}-conv'.format(in_feat, out_feat),
                            nn.Conv2d(in_feat, out_feat, 4, 2, 1, bias=False))
            # feat.add_module('pyramid-{0}-{1}-conv'.format(nc, ndf),
            #                 DepthwiseSeparableConv(in_feat, out_feat, kernel_size=4,
            #                                        kernels_per_layer=2,
            #                                        padding=1))
        ...
        feat.add_module('final-{0}-{1}-conv'.format(cndf, 1),
                        nn.Conv2d(cndf, nz, 4, 1, 0, bias=False))
        # feat.add_module('final-{0}-{1}-conv'.format(nc, ndf),
        #                 DepthwiseSeparableConv(cndf, nz, kernel_size=4,
        #                                        kernels_per_layer=1,
        #                                        padding=0))
...
def define_G(opt, norm='batch', use_dropout=False, init_type='normal'):
    ...
    # netG = UNet(opt.nc)
    # netG = UNet_Attention(opt.nc)
    # netG = UNetDS_Attention(opt.nc)
...
class UNetDS_Attention(nn.Module):
    ...
    def forward(self, x):
        x1 = self.inc(x)
        x1Att = self.cbam1(x1)
        # result1_x = np.array(x1.cpu().detach())
        # io.savemat('save1_x_down.mat', {'result1_x': result1_x})
        x2 = self.down1(x1)
        x2Att = self.cbam2(x2)
        x3 = self.down2(x2)
        x3Att = self.cbam3(x3)
        ...
        # outputRs = logits.permute(0, 2, 3, 1)
        ...
        # savepath = 'attImg/lbot128/' + timestamp + '_r.jpg'
        # image.save(savepath)
        return logits
...
class UnetGenerator(nn.Module):
    ...
    def forward(self, input):
        # return self.model(input), latent
        return self.model(input)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        downconv = nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)
        # Conv to Pool
        # downconv = nn.Sequential(SoftPool2d(), nn.Conv2d(input_nc, inner_nc, kernel_size=1))
    ...
    def forward(self, x):
        # global latent
        # if self.outermost:
        #     self.test = self.model(x) # [torch.cuda.FloatTensor of size 1x512x2x2 (GPU 6)]
        #     return self.test
        # elif self.innermost:
        #     latent = self.model(x)
        #     return torch.cat([latent, x], 1)
        if self.outermost:
            return self.model(x)
        else:
